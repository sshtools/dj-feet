...
public class DBusDaemon implements Closeable, Callable<Integer> {
	...
	public final static class Version implements IVersionProvider {
		...
		public String[] getVersion() throws Exception {
			return new String[] { ArtifactVersion.getVersion("com.sshtools", "dj-feet-daemon") };
		}
	}
	...
	public Integer call() throws Exception {
		...
		transport = TransportBuilder.create(address).configure().withAutoConnect(false)
				.configureSasl().withAuthMode(authMode.orElse(SaslAuthMode.AUTH_ANONYMOUS)).back().back().build();
	...
	void reloadConfig() {
		...
		if(Files.exists(path)) {
			...
			var servicesDir = configuration.getOr("services.d").map(Paths::get).orElseGet(() -> path.getParent().resolve("services.d"));
	...
	private static void saveFile(String data, Path path, boolean insecure) {
	...
	boolean activate(String service) throws DBusException {
		// TODO synchronize somehow so only one connection at a time can start a particular service
		...
		if(srvSection.contains("Exec")) {
			...
			pb.directory(srvSection.getOr("Directory").map(File::new).orElseGet(() -> new File(System.getProperty("user.home"))));
			...
			throw new UnsupportedOperationException(MessageFormat.format("'Exec' directive missing in service {0}", service));
		}
	}
	...
	String getId() {
		return configuration.getOr("id").map(str -> str.equals("auto") ? getDefaultId() : str).orElseGet(() -> 
			getDefaultId()
		);
	}

	String getMachineId() {
		return configuration.getOr("machine-id").map(str -> str.equals("auto") ? getDefaultMachineId() : str).orElseGet(() -> 
			getDefaultMachineId()
		);
	}
	...
	private String getDefaultId() {
		try {
			return Hexdump.toHex(
					MessageDigest.getInstance("MD5").digest((System.getProperty("user.name") + ":" + System.getProperty("user.dir")).getBytes()));
